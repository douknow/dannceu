...
export default function Home() {
  ...
  const exportImage = async () => {
    ...
    // scale = 1
    html2canvas(canvasElement, {scale: 1, backgroundColor: 'transparent'}).then(async (canvas) => {
      // 将canvas转换为图片数据URL
  ...
  return (
    <div className="bg-slate-100">
      <div className="absolute top-0 left-0 w-full h-full z-10 bg-black/70 flex justify-center items-center" style={{ display: exporting ? 'flex' : 'none' }}>
